...
$SkillName = "goosnav-parent-verification"
...
if ($Scope -eq "User") { if ([string]::IsNullOrWhiteSpace($HOME) -or -not (Test-Path -LiteralPath $HOME -PathType Container)) { throw "A valid user home is required" }; $Root = Join-Path $HOME ".agents\skills"; $Base = $HOME }
else { if ([string]::IsNullOrWhiteSpace($RepoPath) -or -not (Test-Path -LiteralPath $RepoPath -PathType Container)) { throw "-RepoPath must be an existing directory" }; $Resolved = [IO.Path]::GetFullPath((Resolve-Path -LiteralPath $RepoPath).Path); if ($Resolved -eq [IO.Path]::GetPathRoot($Resolved)) { throw "Refusing filesystem root" }; $Root = Join-Path $Resolved ".agents\skills"; $Base = $Resolved }
Copy-Skill $Root; Remove-Legacy (Join-Path $Base ".codex\skills"); Remove-Legacy (Join-Path $Base ".claude\skills")
