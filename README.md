...
Reusable Agent Skills for software engineers and founder-operators who need working products, honest evidence, and low-drama operations. The pack combines the existing productization protocol with six focused skills that can be installed together or independently.
...
Each immediate skill directory is independently installable and contains `SKILL.md`, `install.sh`, `install.ps1`, and a local validator under `scripts/`. Detailed guidance lives in each skill's `references/`; the software-productization directory also carries its launcher assets. Pack-level installers discover skill directories by finding every `install.sh` one level down, so adding a new skill needs no change to the pack scripts.
...
macOS or Linux — installs every skill for the current user, no arguments needed:
